...
## 1.1 Explanatory Philosophy & Learning Style

To ensure the notes are highly readable and digestible for the user (who is comfortable with high-level languages like R, SQL, and Python, but struggles with math and middle-level systems jargon):

- **Bridge C to High-Level Languages — R first, Python/SQL second:** When introducing low-level C concepts, reach for an R analogy before a Python or SQL one (e.g., contrasting a C array's fixed size with an R vector's, or a C `struct` with a row of an R `data.frame`). Python or SQL are acceptable fallbacks only when R has no clean equivalent for the concept at hand.
- **Physical/Mechanical Analogies Over Abstract Math:** Avoid abstract mathematical or computer science jargon where possible. Explain hardware behaviors using concrete, everyday analogies (e.g., memory addresses as "house numbers," functions on the stack as "a pile of sticky notes," command-line arguments as a "drive-thru order").
- **Chronological, Line-by-Line Execution Tracing:** When presenting code, do not just explain what the code does as a whole. Break it down into a step-by-step chronological trace of what the CPU executes line-by-line, showing exactly what changes at each step.
- **Lego-Block Concept Building:** Break down complex topics into small, isolated units. Explain one small part of the mechanism (e.g., "what does return do?"), establish understanding of that unit, and only then stack the next unit on top of it.
- **Copy-Pasteable Cheat Sheets:** Conclude explanations with a single, highly structured, clean markdown summary block (cheatsheets, tables, or line-by-line flow charts) that can be easily copied and referenced later.
...
- **Every heading — `##`, `###`, or `####` — is its own standalone markdown cell
  containing only the heading line.** Body content (prose, tables, code fences,
  blockquotes) for that heading goes in one or more separate cells immediately
  following it, ending at the next heading of any level. Subsections use `###`,
  sub-subsections `####`.
  > Rationale: VS Code and Jupyter give each heading-led cell its own fold control,
  > nested by heading level — folding a `##` cell hides everything down to the next
  > `##`; a `###` cell hides down to the next `###`-or-shallower; a `####` cell hides
  > only its own body, since `####` is the deepest level this scheme numbers. Bundling
  > a heading with its body in one cell, or skipping a level, breaks that nesting.

  Example — three consecutive cells, correctly split:
  ```text
  [cell 1 — heading only]
  ### 3.9.2 Selection Sort

  [cell 2 — body only, no heading]
  **Selection sort** repeatedly scans the unsorted portion of an array for
  its smallest remaining value and swaps that value into place at the
  front of that portion. In English:
  ```text
  Repeat until no unsorted elements remain:
      ...
  ```
  [... table, more prose, etc., all still in cell 2 or further plain cells,
   until the next heading ...]

  [cell 3 — heading only, starts the next subsection]
  #### 3.9.2.1 Cost Analysis
  ```
  A cell is malformed if it has a heading on its first line *and* a second line of
  content (that content must move to its own cell), or if two headings appear
  back-to-back with no body between them and the lower one was actually meant to
  start a body paragraph instead.
...
- **Splitting a notebook into one-heading-per-cell is mechanical and easy to corrupt by
  hand** (truncated headings, duplicated paragraphs, stray empty cells were all observed
  from manual splits mid-session). Do it with a script: concatenate all cell source in
  order, walk it line-by-line tracking code-fence state, and cut a new cell every time a
  `#{2,4} ` heading line appears outside a fence — one cell for the heading alone, the
  next for everything up to the following heading. Verify afterward that every cell
  beginning with `#` has *no* second line, and that no text was lost or duplicated (e.g.
  diff a unique marker phrase's occurrence count before/after).
